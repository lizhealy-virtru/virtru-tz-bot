...
def lambda_handler(event, context):
    """
    Handle actions from messages
        - delete the ephemral message
        - post a timezone message if necessary
    """
    ...
    body = event["body"]
    form_json = json.loads(urllib.parse.parse_qs(body)["payload"][0])
    action = form_json["actions"][0]["value"]
    response_url = form_json["response_url"]
    user_id = form_json["user"]["id"]
    channel_id = form_json["channel"]["id"]
    action_value = form_json["actions"][0]["value"]
    message_ts = form_json["container"]["message_ts"]

    return_json = {
                    "delete_original":True
                }
    response = requests.post(url=response_url, json=return_json, headers={'Content-Type': 'application/json'})

    if action_value == "send_message":
        text, user_id = handle_message(message_ts, channel_id)
        #if the sender was not the bot
        if(user_id!=bot_user_id):
            try:
                #construct reply and send
                bot_message = goTZBot(user_id, channel_id, text) # these are my blocks
                bot_message = bot_message[2:-2]
                final_attachment = [{"color": ATTACHMENT_COLOR, "blocks":bot_message}]
                response = slack_web_client.chat_postMessage(channel=channel_id, attachments = final_attachment)
